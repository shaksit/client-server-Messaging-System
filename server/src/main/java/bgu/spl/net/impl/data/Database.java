...
public class Database {
	...
	public LoginStatus login(int connectionId, String username, String password) {
		...
		if (!userExistInDatabase(username)) {
			String sql = String.format( // Log new user registration in SQL
					"INSERT INTO users (username, password, registration_date) VALUES ('%s', '%s', datetime('now'))",
					escapeSql(username), escapeSql(password));
			executeSQL(sql);
			addNewUserCase(connectionId, username, password); // log new user in memory
			// Log login
			logLogin(username);
	...
	private boolean userExistInDatabase(String username) {
		String sql = String.format(
				"SELECT COUNT(*) FROM users WHERE username='%s'",
				escapeSql(username));
		String result = executeSQL(sql);
		// Check for SUCCESS and non-zero count
	...
	private LoginStatus userExistsCase(int connectionId, String username, String password) { // user exists in database
																								// but not in memory
		String sql = String.format(
				"SELECT password FROM users WHERE username='%s'",
				escapeSql(username));
		String result = executeSQL(sql);
		// System.out.println("SQL result for password retrieval: " + result);

		if (result.contains("ERROR")) {
			System.err.println("SQL Error during login for user " + username);
			return LoginStatus.WRONG_PASSWORD; // Treat SQL error as wrong password for safety
		}
		...
		if (result.startsWith("SUCCESS")) {
			...
			if (parts.length > 1) {
				...
				// Remove Python tuple artifacts: ('password',)
				storedPassword = storedPassword.replace("(", "").replace(")", "").replace("'", "").replace(",", "")
						.trim();
			}
		}
		...
		synchronized (userMap) {
			if (userMap.containsKey(username)) {
				...
				if (existingUser.isLoggedIn()) {
					return LoginStatus.ALREADY_LOGGED_IN; // Prevent double login from 2 connection ID's
				}
				// If user exists in map but is logged out we update the existing object.
				existingUser.setConnectionId(connectionId);
				existingUser.login();
				connectionsIdMap.put(connectionId, existingUser);
				return LoginStatus.LOGGED_IN_SUCCESSFULLY;
			}
			// User not in memory, create new
