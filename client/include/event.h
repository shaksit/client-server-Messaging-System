...
public:
  Event(std::string name, std::string team_a_name, std::string team_b_name,
        int time, std::map<std::string, std::string> game_updates,
        std::map<std::string, std::string> team_a_updates,
        std::map<std::string, std::string> team_b_updates,
...
// an object that holds the names of the teams and a vector of events, to be
// returned by the parseEventsFile function
struct names_and_events {
  ...
  names_and_events(std::string team_a, std::string team_b,
                   std::vector<Event> events)
      : team_a_name(team_a), team_b_name(team_b), events(events) {}
};
